...
num_samples = len(filenames_prefixs)
...
while True:
    ...
    ind = random.randint(0, num_samples)

    """
    Rlplacing your own image path here
    Rlplacing your own image path here
    """
    img_path_source = '{}/{}_input.png'.format(dataroot, filenames_prefixs[ind])
    ...
    axarr[2].imshow((outputs.cpu().detach().numpy()[0,...].transpose([1,2,0])[:,:,::-1]*255).astype('uint'))
